...
class IssuesView extends Backbone.View
  ...
  deleteDialog: (e) =>
    ...
    Dialog.create(@dialogConfirm, dialogTitle) 
    return false
          ...
          success: (issues) ->
            if (issues.length == 0)
              ...
              $("#issuesTable tbody").html _(issues).map (issue) ->
                ...
                @$el.append "
                  <tr>
                    <td class='mdl-data-table__cell--non-numeric'><a href='#' class='issue-edit' data-issue-id='#{issue._id}'>#{issue.Description}</a></td>
                    <td class='mdl-data-table__cell--non-numeric'>#{date}</td>
                    <td class='mdl-data-table__cell--non-numeric'>#{issue["Assigned To"] or "-"}</td>
