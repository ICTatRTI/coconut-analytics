...
(doc) ->
  ...
  if doc._id[0..9] is "enrollment"
    if doc.performance
      for student, data of doc.performance
        ...
        for category, score of data
          unless score is "" or category is "Total"
            ...
            parsedLetterScore = score.match(/^([ABCDEXYZ])(\+|-){0,1}$/)

            totalScore+=
              if parsedLetterScore
                [ignore, letterGrade, modifier] = parsedLetterScore
                numberValueForScore = switch letterGrade
                  when "A" then 95
                  when "B" then 85
                  when "C" then 75
                  when "D" then 65
                  ...
                  when "X", "Y", "Z"
                    ...
                    0
                if modifier
                  numberValueForScore += switch modifier
                    when "+" then 5
                    when "-" then -5
                numberValueForScore
              else
        ...
        averageScore = Math.round(totalScore/numberOfCategories)
        ...
        student = student
        emit [year,term,region,school,className,stream,student], averageScore
